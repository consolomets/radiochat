 ...
 * !!!!!!
 * Добавить шифрованню, можно на компе в обычных сях испробовать
...
const char settings8[] PROGMEM = R"=====(">
    ...
    <div>
      <input type="radio" id="server" name="server")=====";
...
<div>
  <input type="number" id="frequency" name="frequency"
         min="410000000" max="525000000" value=")=====";

const char settings15[] PROGMEM = R"=====(">
  <label for="frequency">Частота сигнала (Гц) Frequency</label>
</div>
<div>
...
const char settings155[] PROGMEM = R"=====(">
  <label for="frequency">Ширина канала (Гц) Bandwith (умеет 7800, 10400, 15600, 20800, 31250, 41700, 62500, 125000, 250000, 500000)</label>
</div>
<div>
...
int sygnalfrequency = 433E6; //
int signalbandwith = 125E3;
...
String assid = "Radio.chat.a1";
String aWPA = "Radio.chat.a1";
...
/*    f = SPIFFS.open("/init.bin", "w"); 
    ...
    f.write(sygnalfrequency); //signalBandwidth - signal bandwidth in Hz, defaults to 125E3 (7.8E3, 10.4E3, 15.6E3, 20.8E3, 31.25E3, 41.7E3, 62.5E3, 125E3, 250E3, and 500E3)
...
void handleSubmit() { //проверка ответов? .toInt  //надо всё прожамкать, посмотреть, перевести в типы и схоронять в флешку
  ...
  bool GeneralChanges = false;
  bool TimeChanges = false;
  bool CryptChanges = false;
  bool WiFiChanges = false;
  bool WiFi2Changes = false;
  bool LoRaChanges = false;
  ...
  if (server.hasArg("frequency")) {     
    String msg_text = server.arg("frequency"); 
          debinfo += "<br> frequency: " + msg_text;
          ConfigChanges = true;
    }
    if (server.hasArg("signalbandwith")) {     
    String msg_text = server.arg("signalbandwith"); 
          debinfo += "<br> signalbandwith: " + msg_text;
          ConfigChanges = true;
    }
...
void remove() {  
  String reloadafter = "<html><head><meta http-equiv=\"refresh\" content=\"30; url='/'\" charset=UTF-8\"></head><body><p>Один момент! (30 секунд)</p></body></html>";
...
void handleSettings() {  
  ...
  contentpart = String(sygnalfrequency);
  server.sendContent(contentpart);
...
void factoryreset(bool rewrite){ //fixme добавить логический аргумент и если да, то по дефолту всё, а если нет, то пишем текущие
  if(!rewrite){
    isAP = true; 
    SoundON = true;
    LEDON = true;
  ...
  Wire.begin(TX,RX); 
    ...
    if(SPIFFS.exists("/init.bin")) // проверка есть ли файл
      ...
      if(SPIFFS.exists("/apwpa")){
       ...
       //fixme добавить чтобы пикало айпишник !если включен звук
